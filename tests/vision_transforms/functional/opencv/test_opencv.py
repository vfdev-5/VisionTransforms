...
def test_adjust_hue(F):
    ...
    for a in np.arange(-0.5, 0.5, 0.01):
        ...
        # Abs Tolerance : 10 pixels due to different hue precision computation
        # Pillow in uint8 vs OpenCV in float32
        assert np.allclose(pil_res, cv_res, atol=10), "Failed for {}".format(a)
